...
//This was found online to do complex exponent
__host__ __device__ static __inline__ cuDoubleComplex my_cexpf (cuDoubleComplex z)
{
	cuDoubleComplex res;
	float s, c;
	float t = expf(z.x);
...
__host__ __device__ static __inline__ cuDoubleComplex my_pow(cuDoubleComplex z, cuDoubleComplex a)
{
	cuDoubleComplex w;
	...
	//arga = carg(a); //How to calculate arg?
	arga = atan2(cuCreal(a), cuCimag(a));
	//printf("arga is %0.4f\n", arga);
	r = pow(absa, x);
	//printf("r is %0.4f\n", r);
	theta = x * arga;
	...
	w = make_cuDoubleComplex(r * cos(theta), + (r * sin(theta)));
	return w;
}

__host__ __device__ static __inline__ cuDoubleComplex my_exp_pow(double x, double n)
{
	//both x and n are always real
	//x is always -2*pi/N
	//n is always an integer
	double nx = n*x;
	double cos_arg = cos(nx); //this could be done with sincosf
	double sin_arg = sin(nx);

	return make_cuDoubleComplex(cos_arg, sin_arg);
}
